...
const exportModule = () => {
  ...
  const encrypt = async (data, key, counter) => {
    if (typeof key === 'string' || Array.isArray(counter)) {
      key = Buffer.from(key);
    }
    if (key.length !== 32) {
      throw new TypeError('invalid key length ' + key.length);
    }
    if (counter.buffer.byteLength !== 16) {
      throw new TypeError(
        'invalid counter (iv) length ' + counter.buffer.byteLength,
      );
    }

    counter = Buffer.from(counter); // copy
    ...
    const keyInts = convertToInt32(key);

    for (let i = 0; i < 8; i++) {
      KE[i >> 2][i % 4] = keyInts[i];
    }

    let RPointer = 0;
    let t = keyInts.length;
    let tt;
    while (t < 60) {
      tt = keyInts[keyInts.length - 1];
      keyInts[0] ^=
        (S[(tt >> 16) & 0xff] << 24) ^
        (S[(tt >> 8) & 0xff] << 16) ^
        (S[tt & 0xff] << 8) ^
      ...
      tt = keyInts[keyInts.length / 2 - 1];

      keyInts[keyInts.length / 2] ^=
        S[tt & 0xff] ^
        (S[(tt >> 8) & 0xff] << 8) ^
        (S[(tt >> 16) & 0xff] << 16) ^
        (S[(tt >> 24) & 0xff] << 24);

      for (let i = keyInts.length / 2 + 1; i < keyInts.length; i++) {
        keyInts[i] ^= keyInts[i - 1];
      }

      // copy values into round key arrays
      for (let i = 0; i < keyInts.length && t < 60; t++, i++) {
        KE[t >> 2][t % 4] = keyInts[i];
      }
    }
    ...
    const encrypted = Buffer.from(data); // copy
    ...
    for (let i = 0; i < encrypted.length; i++) {
      if (remainingCounterIndex === 16) {
        ...
        for (let r = 1; r < 14; r++) {
          for (let i = 0; i < 4; i++) {
            a[i] =
              T[0][(dataInts[i] >> 24) & 0xff] ^
              T[1][(dataInts[(i + 1) % 4] >> 16) & 0xff] ^
              T[2][(dataInts[(i + 2) % 4] >> 8) & 0xff] ^
        ...
        remainingCounter = Buffer.alloc(16);
        let tmp;

        for (let i = 0; i < 4; i++) {
          tmp = KE[14][i];
          remainingCounter[4 * i] =
            (S[(dataInts[i] >> 24) & 0xff] ^ (tmp >> 24)) & 0xff;
          remainingCounter[4 * i + 1] =
            (S[(dataInts[(i + 1) % 4] >> 16) & 0xff] ^ (tmp >> 16)) & 0xff;
          remainingCounter[4 * i + 2] =
            (S[(dataInts[(i + 2) % 4] >> 8) & 0xff] ^ (tmp >> 8)) & 0xff;
          remainingCounter[4 * i + 3] =
            (S[dataInts[(i + 3) % 4] & 0xff] ^ tmp) & 0xff;
        }
        remainingCounterIndex = 0;
        incrementBuffer(counter);
      }
